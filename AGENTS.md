...
- Build/run the app: it is a CLI, not a server. Core commands: `finalstrike --version`, `finalstrike doctor --repo fixtures/sample-app`, `finalstrike validate-config --repo fixtures/sample-app`, and `finalstrike plan --repo fixtures/sample-app --acceptance fixtures/sample-app/acceptance-smoke.md --dry-run`. See `README.md` Quick start.
