...
- The `fixtures/sample-app` server (integration target) runs via `python -m sample_app.server 8080` from inside `fixtures/sample-app/` and serves `GET /health` -> `200 ok`, `GET/POST /api/tasks` (in-memory task store). The static frontend on port 3000 includes `/tasks/` (title "Sample App - Tasks") for Tier 1 computer-use demos.
