...
def test_smoke_title_scenario_with_action_cassette(
    ...
    responses = [
        raw.replace("http://localhost:8080/", static_frontend_server)
...
def test_full_tasks_title_scenario_with_action_cassette(
    ...
    tasks_url = f"{static_frontend_server.rstrip('/')}/tasks/"
    responses = [
        raw.replace("http://localhost:8080/tasks/", tasks_url)
        for raw in cassette.responses
    ]
    ...
    instruction = (
        f'Open {tasks_url} and verify the page title is "Sample App - Tasks"'
    )
    output_root = tmp_path / "runs" / "full-tasks-run"
    loop = ActionLoop(
        instruction=instruction,
        output_dir=output_root,
        provider=ReplayActionProvider(responses),
