...
def test_implemented_phases_cover_p0_through_p3() -> None:
    assert IMPLEMENTED_PHASES == frozenset({0, 1, 2, 3})
    assert next_unimplemented_phases()[0] == 4


def test_stub_modules_reference_future_phases() -> None:
    phases = {item.phase for item in STUB_MODULES}
    assert phases >= {4, 5, 6, 7, 8, 9, 10}
